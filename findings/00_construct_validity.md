...
MirrorCode measures, precisely: *can a frontier model — with prior exposure to (screen-positive on) these specific public
...
alone" cannot be separated from recall on the released data for the two-thirds of targets that are
screen-positive (gap 2). The construct is narrower
...
| **decay** | what survives publication? | **weak** — targets are famous public programs, 17/25 memorized; the recall contribution to the score is unquantifiable from released data ([finding 03](03_how_much_is_recall.md)); canary strings help detect, not prevent |
...
**The constructive fix** — the metric that would answer the marketing question honestly a preregistered
size × time-to-success × whole-task-success profile) — is [finding 05](05_recommendation.md).
